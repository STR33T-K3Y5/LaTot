...
const Home = () => {
  ...
  const aboutTouchEndX = useRef(null);
  const aboutItemsRef = useRef(null);
  const aboutAnimationTimeout = useRef(null);
  const ANIMATION_DURATION = 500; // Match CSS duration
  ...
  // About navigation handlers
  const goToAboutSlide = (index, direction = null) => {
    ...
    let newIndex = index;
    if (newIndex < 0) {
      newIndex = aboutTotalSlides - 1;
    } else if (newIndex >= aboutTotalSlides) {
      newIndex = 0;
    }
    
    // Determine direction if not provided
    const animDirection = direction || (newIndex > aboutCurrentIndex ? 'right' : newIndex < aboutCurrentIndex ? 'left' : null);
    
    setAboutDirection(animDirection);
    setAboutAnimating(true);
    setAboutCurrentIndex(newIndex);
    
    // Reset animation state after animation completes
    if (aboutAnimationTimeout.current) {
      clearTimeout(aboutAnimationTimeout.current);
    }
    aboutAnimationTimeout.current = setTimeout(() => {
      setAboutAnimating(false);
      setAboutDirection(null);
    }, ANIMATION_DURATION); // Match CSS transition duration
  };

  const nextAboutSlide = () => {
    goToAboutSlide(aboutCurrentIndex + 1, 'right');
  };

  const prevAboutSlide = () => {
    goToAboutSlide(aboutCurrentIndex - 1, 'left');
  };
